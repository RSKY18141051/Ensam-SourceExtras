;1USUARIOS                     -
;RIOS VAZQUEZ RAZIEL           - 
;SANCHEZ MEDINA PAOLA GUADALUPE-
;20 NOVIEMBRE 2020             -
;-------------------------------
...
input1 db "Ingresar usuario: ",24h
input2 db "Ingresar clave: ",24h 

msg1 db "Usuario inexistente...",24h;mensaje si el usuario no existe
msg2 db "Clave incorrecta...",24h   ;mensaje si la clave es incorrecta
msg3 db "Acceso permitido!",24h     ;mensaje auxiliar    

rutau db "c:\usuarios.txt",0        ;ruta del archivo de texto usuarios
rutac db "c:\claves.txt",0          ;ruta del archivo de texto claves

handle dw ?

usuario db 20 dup(?)                ;usuario ingresado desde teclado
clave db 20 dup(?)                  ;clave ingresada desde teclado

auxu db 35 dup(?)                   ;variable auxiliar de usuarios 
auxc db 45 dup(?)                   ;variable auxiliar de claves

usuario1 db 15 dup(?)               ;usuario1 del txt
usuario2 db 15 dup(?)               ;usuario2 del txt
usuario3 db 15 dup(?)               ;usuario3 del txt
usuario4 db 15 dup(?)               ;usuario4 del txt
   
clave1 db 15 dup(?)                 ;clave1 del txt
clave2 db 15 dup(?)                 ;clave2 del txt
clave3 db 15 dup(?)                 ;clave3 del txt
clave4 db 15 dup(?)                 ;clave4 del txt
...
start:
    ...
    ;se posiciona al inicio de usuario
    lea si,usuario
    ;pide usuario 
    imprime input1
...
;si el usuario no existe imprime el mensaje de inexistente
inexistente:
    ;nueva linea
    imprimec 13
    imprimec 10 
...
;si la clave es incorrecta imprime el mensaje de incorrecta    
incorrecta:
    ;nueva linea
    imprimec 13
    imprimec 10 
...
;si el usuario y la clave existen, da acceso
ingresa:
    ;nueva linea
    imprimec 13
    imprimec 10 
...
;------------------------------------PROCEDIMIENTOS----------------------------------------
;lee un caracter desde teclado y lo guarda en al
leec proc near
    mov ah,01
    int 21h
    ret
